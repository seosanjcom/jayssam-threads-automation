...
function productSceneLines(input) {
  ...
  if (/링티/.test(combined)) {
    return [
      ...
      variant("lingtea-cta", [
        "댓글에 제품 정보와 섭취 안내를 모아둘게~ 외출 가방에 넣어둘지 천천히 보고 골라.",
        "링크랑 기본 정보는 댓글에 적어둘게~ 집에서 필요한 상황인지 먼저 살펴보면 돼.",
        "댓글에 확인할 정보만 깔끔히 두고 갈게!! 필요할 때 라벨까지 보고 판단해줘.",
      ]),
    ];
  }
  if (/아이\s*전용|어린이용\s*(?:음료|식품)/.test(combined)) {
    return [
      ...
      variant("child-cta", [
        "댓글에 상품 정보는 적어둘게~ 필요할 때 라벨까지 같이 보고 결정해.",
        "링크는 댓글에 두고 갈게!! 급하게 고르기보다 섭취 조건부터 살펴봐.",
        "댓글에 기본 정보만 정리해둘게~ 집에서 필요한 상황인지 먼저 생각해보면 돼.",
      ]),
    ];
  }
  if (/머리끈/.test(combined)) {
    return [
      ...
      variant("hair-cta", [
        "댓글에 수량이랑 링크 적어둘게!! 쓸 곳부터 떠올려보고 담아.",
        "고무줄 자꾸 사라지는 집이면 댓글 정보 보고 필요한 만큼만 챙겨~",
        "댓글에 상품 정보 모아둘게~ 재질이 맞는지 보고 골라주면 돼!!",
      ]),
    ];
  }
  if (/케이블|충전/.test(combined)) {
    return [
      ...
      variant("cable-cta", [
        "댓글에 규격이랑 링크 남겨둘게!! 책상에 붙일 자리가 있는지만 보고 골라.",
        "선 줍는 일이 귀찮았던 사람은 댓글 정보에서 고정 방식부터 살펴봐~",
        "댓글에 상품 정보 적어둘게~ 쓰는 케이블 굵기랑 맞는지 보고 담아!!",
      ]),
    ];
  }
  if (/파우치|가방/.test(combined)) {
    return [
      ...
      variant("pouch-cta", [
        "댓글에 크기랑 링크 적어둘게!! 가방 안 자리가 남는지 보고 골라.",
        "작은 물건 찾는 시간이 아까웠다면 댓글 정보에서 수납칸부터 봐줘~",
        "댓글에 상품 정보 모아둘게!! 보조배터리까지 들어가는지 확인하고 담아.",
      ]),
    ];
  }
  if (/청소|행주|욕실|주방|수세미/.test(combined)) {
    return [
      ...
      variant("cleaning-cta", [
        "댓글에 설치 방식이랑 링크 적어둘게!! 싱크대에 놓을 자리부터 재봐.",
        "설거지 뒤 둘 곳이 늘 고민이었다면 댓글 정보에서 고정 방법부터 살펴봐~",
        "댓글에 상품 정보 모아둘게!! 물 빠짐이 편한 구조인지 보고 골라.",
      ]),
    ];
  }
  if (/트레이|수납|바구니|옷걸이|연결\s*고리/.test(combined)) {
    return [
      ...
      variant("storage-cta", [
        "댓글에 크기랑 링크 적어둘게!! 놓을 자리 재본 뒤 골라.",
        "잔물건 찾는 시간을 줄이고 싶다면 댓글 정보에서 구성부터 살펴봐~",
        "댓글에 상품 정보 모아둘게!! 지금 정리할 물건이 들어갈지 보고 담아.",
      ]),
    ];
  }
  if (operatorNote) {
    return [
      ...
      variant("operator-guidance", [
        "설치할 자리와 필요한 수량을 먼저 생각해보고, 내 동선에 맞을 때만 골라줘.",
        "제품 크기와 구성은 댓글 링크에서 보고, 쓸 장면이 떠오를 때만 담아~",
        "비슷해 보여도 사용 환경이 다르니, 놓을 곳과 규격부터 천천히 봐줘.",
      ]),
      variant("operator-cta", [
        "댓글에 상품 정보 정리해둘게!! 필요한 부분만 골라서 보면 돼.",
        "링크랑 기본 정보는 댓글에 적어둘게~ 내 공간에 맞는지 보고 결정해.",
        "댓글에 확인할 정보만 모아둘게!! 급하게 담기보다 쓰임부터 생각해봐.",
      ]),
    ];
  }
  return [
    ...
    variant("fallback-guidance", [
      "설치할 자리와 필요한 수량을 먼저 생각해보고, 내 생활에 맞을 때만 골라줘.",
      "제품 크기와 구성은 댓글 링크에서 보고, 쓸 장면이 떠오를 때만 담아~",
      "비슷해 보여도 사용 환경이 다르니, 놓을 곳과 규격부터 천천히 봐줘.",
    ]),
    variant("fallback-cta", [
      "댓글에 상품 정보 정리해둘게!! 필요한 부분만 골라서 보면 돼.",
      "링크랑 기본 정보는 댓글에 적어둘게~ 내 공간에 맞는지 보고 결정해.",
      "댓글에 확인할 정보만 모아둘게!! 급하게 담기보다 쓰임부터 생각해봐.",
    ]),
...
function buildComments(input, productLinks) {
  ...
  if (productLinks.length) {
    if (input.content_mode === "found_product" || input.content_mode === "recommendation") {
      ...
      comments.push([
        productCopyVariant(input, "comment-heading", [
          "댓글에 상품 정보 모아둘게!!",
          "링크랑 기본 정보는 여기 적어둘게!!",
          "제품 정보는 댓글에 깔끔히 정리해둘게!!",
        ]),
        ...verifiedDetails,
