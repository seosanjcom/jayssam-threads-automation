...
OBSERVATION_SEEDS = [
    ("excel_certificate_gap", "실무 파일 앞에서 멈추는 진짜 이유", "‘선생님, 자격증은 땄는데 실무 파일 보니까 손도 못 대겠어요.’\n수업하다 보면 진짜 자주 듣는 소리다.\n\n당연하지. 시험은 깔끔한 표 주고 ‘VLOOKUP 쓰세요’ 알려주지만, 회사 데이터는 셀 병합에 오타, 공백 범벅인 쓰레기 데이터를 던져주니까.\n\n자격증이 도구 위치 외운 거라면 실무는 고장 난 기계 고치는 거다. 처음 보는 파일 받았을 때 수식부터 넣지 말고 Ctrl + G 눌러서 빈 셀부터 찾아라. 병합 셀 풀고 텍스트로 돌아간 숫자 잡는 게 먼저다."),
    ...
    ("photoshop_pretty_not_sell", "예쁜 디자인보다 먼저 봐야 할 것", "가게 하시는 사장님들이 제일 많이 하시는 질문.\n‘화려하게 디자인해서 광고 돌렸는데 왜 손님이 안 오죠?’\n\n포토샵은 그림 예쁘게 다듬는 도구일 뿐이지, 지갑 열게 만드는 건 ‘이게 나한테 왜 필요한가’다.\n\n툴 기능 외우려고 애쓰지 마라. 한 페이지에 폰트 3~4개씩 알록달록 섞어 쓰면 전단지 같아서 바로 스킵당한다. 제일 중요한 한 문장 빼고 폰트 종류부터 2개로 줄여봐라."),
    ...
    ("youtube_title_promise", "유튜브 제목은 낚시가 아니라 약속이다", "유튜브 제목 어그로 끌어서 올리면 당장 조회수는 나올지 모른다.\n\n근데 내용이 못 따라가면 속았다는 생각에 그 채널 다시는 안 본다. 제목은 낚싯바늘이 아니라 시청자한테 하는 약속이다.\n\n나는 수강생들 편집 프로그램 켜기 전에 제목부터 못 뽑게 한다. ‘이 영상 보는 사람 문제 하나라도 확실하게 해결해 줄 수 있냐?’ 이 질문에 답 못 하면 컷편집이고 자막이고 다 의미 없다."),
    ...
    ("career_certificate_question", "자격증을 따려는 목적부터 물어야 하는 이유", "진로 특강 가면 제일 많이 나오는 질문.\n‘무슨 자격증 따야 취업 잘돼요?’\n\n그럼 난 역으로 물어본다. ‘너 그거 따서 정확히 어디다 쓸 건데?’\n목적도 없이 남들 다 따니까 따라 따는 자격증은 그냥 종이 쪼가리다. 회사는 자격증 개수를 보는 게 아니라 일을 할 줄 아냐를 본다.\n\n가고 싶은 회사 채용공고 3개만 찾아서 우대사항에 적힌 툴이 뭔지부터 봐라. 학원 등록은 그 다음에 하는 거다."),
...
PRACTICAL_GUIDES = [
    (("excel_",), [
        "실무 파일 받으면 Ctrl + G 눌러서 빈 셀부터 찾아라. 병합 셀, 텍스트 숫자부터 잡아야 수식 넣어도 덜 꼬인다.",
        "수식 넣기 전에 숫자가 왼쪽 정렬돼 있는지부터 봐. 텍스트로 저장된 숫자면 함수가 맞아도 결과가 틀어진다.",
    ], [
        "💬 실무 엑셀 파일 열었을 때 제일 당황했던 순간 있으면 댓글로 남겨봐.",
        "📌 오늘 쓰는 파일 하나 열고 빈 셀, 병합 셀부터 봐라.",
        "💬 #N/A 때문에 막혔던 적 있으면 어떤 상황이었는지 남겨봐.",
    ]),
    (("ppt_",), [
        "슬라이드마다 ‘그래서 뭘 결정하면 되지?’를 적어봐. 답이 없는 페이지는 먼저 덜어내야 한다.",
        "발표 전 슬라이드 한 장을 3초만 봐라. 결론 안 보이면 문장부터 쳐내야 한다.",
    ], [
        "📌 지금 만드는 PPT 한 장 열어서 봐라. ‘그래서 어떻게 하자는 건데?’에 답 안 나오면 문장부터 쳐내야 한다.",
    ...
    (("sns_", "smallbiz_", "marketing_"), [
        "올리기 전에 가격·위치·예약 방법부터 봐라. 그거 안 보이면 예쁜 이미지도 그냥 지나간다.",
        ...
        "📌 손님 입장에서 홍보물 한 번 읽어봐. 가격·위치·예약 방법 안 보이면 그거부터 넣어라.",
    ]),
    (("career_", "jobtalk_"), [
...
SPECIFIC_PRACTICAL_GUIDES = {
    "excel_certificate_gap": (
        "",
        "💬 다들 회사에서 엑셀 파일 처음 열었을 때 제일 어이없었던 순간이 언제였냐?",
    ),
    "photoshop_pretty_not_sell": (
        "",
        "📌 지금 만드는 카드뉴스나 포스터 있으면 폰트 몇 개나 썼나 당장 봐라.",
    ),
    "youtube_title_promise": (
        "",
        "💬 영상 제목에 낚여서 들어갔다가 10초 만에 뒤로 가기 누른 적 있지 않냐?",
    ),
    "career_certificate_question": (
        "",
        "📩 가고 싶은 직무나 분야 있으면 댓글 달아봐. 진짜 필요한 툴이 뭔지 알려줄 테니까.",
    ),
}
...
def write_draft(topic: dict, date_text: str, slot: str) -> Path:
    ...
    draft = {
        ...
        "editorial_rules": {
            "voice": "원장님이 수강생에게 바로 말하듯 툭 던지는 짧고 직설적인 현장 구어체",
            "structure": "실제 수강생 질문 또는 실수 → 왜 현장에서 막히는지 → 바로 써먹는 팁 → 대화하듯 던지는 댓글·행동 유도",
            "avoid": ["아동·학부모 소재", "뉴스 요약", "AI 설명체", "학습지식 해설", "짚어볼게·체크해봐류 템플릿", "과장된 약속", "같은 종결어 반복"],
