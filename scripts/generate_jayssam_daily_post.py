...
OBSERVATION_SEEDS = [
    ("excel_certificate_gap", "실무 파일 앞에서 멈추는 진짜 이유", "‘선생님, 자격증은 땄는데 왜 실무 과제는 손을 못 대겠죠?’\n\n수업하다 보면 정말 자주 듣는 말이다. 시험은 ‘여기서 이 함수를 쓰세요’ 하고 답을 알려주지만, 실무는 아무 설명 없이 엉망인 매출 데이터를 던져주고 해결하라고 한다.\n\n자격증이 도구함 위치를 외운 거라면, 실무는 고장 난 기계를 직접 고치는 작업이다. 그래서 나는 합격증을 쥐여주는 것보다 처음 보는 파일 앞에서 어디부터 건드려야 할지 판단하는 기준을 훨씬 오래 시킨다."),
    ...
    ("photoshop_pretty_not_sell", "예쁜 디자인보다 먼저 봐야 할 것", "가게 하시는 사장님들께 포토샵을 가르치다 보면 이런 말을 많이 듣는다. ‘화려하게 만들어서 광고도 돌렸는데 왜 손님이 안 올까요?’\n\n당연하다. 포토샵은 이미지를 예쁘게 다듬어줄 뿐이고, 손님 지갑을 여는 건 내 상품이 왜 필요한지 알려주는 설득력이다.\n\n툴 기능 몇 개 더 외우기 전에 내 손님이 진짜 듣고 싶어 하는 말이 뭔지 파악하는 게 먼저다. 디자인은 그 메시지를 제대로 전달하기 위한 도구일 뿐이다."),
    ...
    ("youtube_title_promise", "유튜브 제목은 낚시가 아니라 약속이다", "유튜브 제목을 자극적으로 쓰면 당장 조회수는 나올지 모른다.\n\n하지만 제목으로 기대를 키워놓고 내용이 못 따라가면, 시청자는 속았다는 느낌에 다시는 그 채널을 안 본다. 제목은 사람을 낚는 낚싯바늘이 아니라, 영상 안에서 반드시 지키겠다는 약속이어야 한다.\n\n그래서 나는 제목부터 뽑지 않는다. 이 영상이 시청자의 무슨 문제를 해결해 줄 수 있는지, 그 확실한 한 가지부터 정리하게 만든다."),
    ...
    ("career_certificate_question", "자격증을 따려는 목적부터 물어야 하는 이유", "진로 특강 가면 ‘무슨 자격증 따야 취업 잘돼요?’라는 질문을 진짜 많이 받는다.\n\n그럴 때 나는 무슨 자격증이 좋은지 답해주기 전에 역으로 묻는다. ‘너 그 자격증 따서 정확히 뭐에 쓰려고 하는데?’\n\n자격증은 목적이 아니라 수단이다. 내가 가려는 분야에서 어떤 도구가 필요한지 목표가 명확해야 자격증도 의미가 있다. 남들 다 따니까 일단 따는 자격증은 이력서 한 줄 채우는 종이 쪼가리로 끝나기 쉽다."),
