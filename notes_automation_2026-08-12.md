...
## 2026-08-13 쿠팡 파트너스 카테고리 상위 상품 확인

- 공개 API 구현 문서에서 카테고리별 베스트 상품 조회 경로 `GET /products/bestcategories/{categoryId}`를 확인했다. 카테고리별 최대 100개 결과가 가능하며, 라이프매거진은 상위 1~5개만 후보 풀로 사용한다.
- 현재 구현된 검색 API는 검색어별 최대 10개만 반환하며 카테고리 베스트 순위를 사용하지 않는다. 상품 선별기를 베스트 카테고리 우선 + 검색 API 폴백 구조로 개편해야 한다.
- 자동화는 카테고리별 베스트 조회를 하루에 제한된 수만 호출해야 한다. 후보 풀이 부족할 경우 억지 발행하지 말고 보류·알림 처리한다.
- 근거 확인 페이지: https://github.com/seungwonme/n8n-nodes-coupang-partners (2026-08-13 확인)
- 카테고리 코드와 베스트 상품 요청 형식은 공개 구현 저장소의 `nodes/coupang-partners/Descriptions/products.ts` 및 `GenericFunctions.ts`에 정의되어 있음을 확인했다. 이 경로의 실제 요청 형식을 기준으로 내부 API 모듈을 확장한다.
